...
Galera Cluster ist eine synchrone Multi-Master- Datenbank-Cluster , basierend auf synchrone Replikation und Oracle MySQL / InnoDB . Wenn Galera Cluster in Betrieb ist , können Sie direkt liest und schreibt zu jedem Knoten , und Sie können jede einzelne Knoten ohne Unterbrechung in Betrieb und ohne die Notwendigkeit, komplexe Failover- Verfahren handhaben zu verlieren. Galera Cluster ist eine einfach zu bedienende, Hochverfügbarkeitslösung , die hohe Systemverfügbarkeit , kein Datenverlust und Skalierbarkeit für zukünftiges Wachstum bietet .


Hilfe Befehl
-----------------

Der Befehl help führt den Anwender über den Zweck und als auch über die Möglichkeiten , die in der mysqlservergalera Modul enthalten sind. Es listet einige der alternativen Parameter mysqlservergalera Modul . Es beschreibt auch die Syntax für die Installation des mysqlgqlera Moduls. Der Befehl help für mysqlservergalera Modul wird wie unten dargestellt.
...
Der Befehl für die Installation der Hafenarbeiter -Modul auf dem Terminal verwendet wird, aufgeführt unten ,
...
Galera Cluster stellt eine signifikante Verbesserung der Hochverfügbarkeit für MySQL- Ökosystem. Die verschiedenen Möglichkeiten, um Hochverfügbarkeit zu erreichen, haben in der Regel nur einen Teil der durch Galera Cluster verfügbaren Funktionen zur Verfügung, sodass die Wahl eines Hochverfügbarkeitslösung eine Übung in der Vor- und Nachteile .

Die folgenden Funktionen sind über Galera Cluster zur Verfügung :

* Echte Multimaster Lesen und Schreiben zu jedem Knoten zu jeder Zeit.
* Synchrone Replikation Kein Sklave lag , werden keine Daten an dem Knoten Absturz verloren.
* Die eng gekoppelte Alle Knoten den gleichen Zustand zu halten. Keine divergierenden Daten zwischen den Knoten erlaubt.
* Multi -Thread- Slave Für eine bessere Leistung . Für jede Arbeitslast .
* Kein Master-Slave- Failover Betrieb oder Einsatz von VIP .
* Hot Standby Keine Ausfallzeiten bei einem Failover ( da es keinen Failover) .
* Automatische Node Provisioning Nein müssen manuell eine Sicherungskopie der Datenbank und kopieren Sie sie auf den neuen Knoten .
* Unterstützt InnoDB .
* Transparent für Anwendungen benötigt keine (oder minimalen ) Änderungen ) an die Anwendung.
* Keine Lese- und Schreib Splitting benötigt .
